...
// Убеждаемся, что поля не пустые
            ...
            boolean i = false;
            int a = 0;
            for (int t = 0; t < destinationAddress.length(); t++) //
            {

                if(destinationAddress.charAt(t) == '.') // проверяем является ли каждый символ стороки точка
                {
                    a++; // если является увеличиваем на 1
                }

                if(destinationAddress.lastIndexOf(".") == destinationAddress.length()-1
                        || destinationAddress.indexOf(".") == 0) // является первый и последний символ точка
                {
                    i = true; // если является  i = true
                    break;
                }

                if(destinationAddress.length() > 255 && destinationAddress.length() <255)
                {
                    i = true;
                    break; // если символ не является цифрой и символ не является точкой то тогда переменной присваевается значение тргу
                }




                if (Character.isDigit(destinationAddress.charAt(t)) == false && destinationAddress.charAt(t) != '.')
                {
                    i = true;
                    break; // если символ не является цифрой и символ не является точкой то тогда переменной присваевается значение тргу
                }
            ...
            if (i == true || a != 3) // 2 пеменные
            {
                JOptionPane.showMessageDialog(this,
                        "Неправильный адрес получателя", "Ошибка",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
